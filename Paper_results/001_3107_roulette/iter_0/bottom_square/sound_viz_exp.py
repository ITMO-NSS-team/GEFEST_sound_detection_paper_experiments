    ...
    for i,ax in enumerate(axs.flat):
        if i < len(spls_opt):
            if i == 0:
                ...
                ax.set_title(f'{sum([len(i) for i in micro[i-1]])} receivers, Loss :{round(best_fit[i-1],2)}')
            ...
            for fit in range(len(best_fit)):
                ...
                ax.plot(best_fit[fit], label=f'{sum([len(pnt) for pnt in micro[fit]])} receivers')
