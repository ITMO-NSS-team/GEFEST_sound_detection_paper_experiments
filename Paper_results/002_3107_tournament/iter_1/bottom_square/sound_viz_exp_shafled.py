...
for a in range(lenght):
    ...
    min_fitness = [round(f[0],3) for f in fitness]
    ind = min_fitness.index(min(min_fitness))
    #fitness = list(np.array(fitness)/np.max(np.array(fitness)))#Normed loss
    best_fit.append(round(fitness[ind][0],3))
    optimized_paths_.append(f"History_{a}/population_{ind}.pickle")
#--#
ls = os.listdir(path='.')
lenght = 0
for i in ls:
...
if __name__ == "__main__":
    ...
    optimized_structures = [upload_file(i)[1] for i in optimized_paths_]
    ...
    for i,ax in enumerate(axs.flat):
        if i < len(spls_opt):
            if i == 0:
                ax.set_title('Reference object')
            elif i<len(spls_opt):
                ax.set_title(f'{receivers[i-1]} receivers, MAE Loss :{round(best_fit[i-1],2)}')
            ...
            for fit in range(len(best_fit)):
                ...
                plt.ylabel("Normed quality",labelpad=0)
            plt.minorticks_on()
            plt.legend()
